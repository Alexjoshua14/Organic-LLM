 ...
 */
export default function MorphTestContainerGPU({ config = DEFAULT_SPRING_CONFIG }: MorphTestContainerProps) {
  ...
  const webGLRef = useRef<Mesh>(null);
  ...
  const { elementRef, reset, morphTo } = useMorphPhysics({ config, webGLRef })
  ...
  return (
    ...
    >
      ...
      {state === MorphState.Red
        ? <RedSquare ref={elementRef} className="bg-transparent" />
        : state === MorphState.Blue ?
          <BlueRectangle ref={elementRef} className="bg-transparent" />
          : <TallRectangle ref={elementRef} className="bg-transparent" />
      }
