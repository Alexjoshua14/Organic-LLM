...
export type StrataAgentThreadScope = { kind: "hub" } | { kind: "page"; pageId: string };
...
export function strataAgentThreadPath(scope: StrataAgentThreadScope): string {
  return scope.kind === "hub"
    ? "/sandbox/prototypes/strata"
    : `/sandbox/prototypes/strata/${scope.pageId}`;
}
