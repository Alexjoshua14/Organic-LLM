...
Sensitivity wins. A product decision that reveals unshipped direction goes to `.context/hub/`
...
| Private — `.context/hub/` or Notion | Public — `docs/` |
|---|---|
...
When genuinely unsure, default to `.context/hub/` and link a neutral pointer from `docs/`.
...
|-------------|----------------|-------------|---------|
| Vision, feel, motivation, assets | Notion (Phase 4+); `.context/hub/` until then | Notion MCP / file edit | Why Speak should feel like a colleague, not a tool |
| Product strategy, moat, roadmap | `.context/hub/` | File edit (gitignored) | Which capabilities are differentiators vs. baseline |
| Product intent, scope, ACs, success metrics | `.context/hub/<feature>/product-spec.md` | File edit (gitignored) | "Voice turns persist to the thread" |
| Unresolved product direction | `.context/hub/<feature>/open-questions.md` | File edit (gitignored) | "One eternal voice thread, or fresh per session?" |
...
1. **Sensitive by the test above?** → `.context/hub/`. Stop here.
...
5. **What we intend to build, or what "done" means?** → `.context/hub/<feature>/product-spec.md`.
