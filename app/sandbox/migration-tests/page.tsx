  ...
  const inner = (
    <>
      ...
      {info.deltaVsLowestReturned != null ? (
        ...
        <p className="mt-1 text-left text-[8px] font-normal tabular-nums text-muted-foreground">
          {metricsPlain}
        </p>
      )}
    </>
  ...
  return (
    <div
      className={cn(
        ...
        flush ? "mt-1 border-0 pl-0" : "mt-3 border-l-2 border-primary/30 pl-2",
        className
      )}
    >
      {inner}
...
function LegacyMarginalCallout({
  ...
  return (
    <div
      className={cn(
        "w-full text-muted-foreground",
        flush ? "mt-1 border-0 pl-0" : "mt-3 border-l-2 border-muted-foreground/25 pl-2",
        className
      )}
    >
      {inner}
          ...
          <PageContentFrame maxWidth="5xl">
            ...
            >
              ...
              <p className="text-xs text-muted-foreground leading-snug mb-2 max-w-2xl">
                ...
                ({MEMORY_PRODUCTION_EMBEDDER_MODEL}, {MEMORY_PRODUCTION_EMBEDDING_DIMS}-D). Cosine
                scores rank within a column only. Hover or focus a cell and use the info icon for
                details.
              </p>
              ...
              <div className="mb-1.5 flex flex-wrap items-center justify-between gap-x-2 gap-y-1">
                ...
                >
                  <span className="text-2xs font-medium uppercase tracking-wide text-primary/90 underline decoration-primary/30 underline-offset-2">
                    Reading tips
                  </span>
                </HintCell>
              </div>
                    ...
                    <Fragment key={row.label}>
                      <div className="border-b border-border/40 px-2.5 py-1.5 text-foreground md:border-r-0">
                        ...
                        <HintCell
                          ...
                          ariaLabel="Explain this setting"
                        >
                      ...
                      <div className="border-b border-border/40 bg-muted/10 px-2.5 py-1.5 text-foreground/90 md:border-l md:border-border/50">
                        ...
                        <HintCell
                          tip={row.production.tooltip}
                          className="-mx-1 px-1 py-0.5"
                          ariaLabel="Explain legacy value"
                        >
                      ...
                      <div className="border-b border-border/40 bg-primary/[0.04] px-2.5 py-1.5 text-foreground/90 md:border-l-2 md:border-primary/25">
                        ...
                        <HintCell
                          tip={row.candidate.tooltip}
                          className="-mx-1 px-1 py-0.5"
                          ariaLabel="Explain candidate value"
                        >
              ...
              <p className="text-2xs leading-snug text-muted-foreground mb-2">
                Live Mem0:{" "}
                <code className="rounded bg-muted/80 px-1 py-0.5 font-mono text-2xs">
                  config/memory-production-meta.ts
                </code>
                {" · "}
                <code className="rounded bg-muted/80 px-1 py-0.5 font-mono text-2xs">
                  config/mem0-config.ts
                </code>
                {" · "}Legacy sandbox constants:{" "}
                <code className="rounded bg-muted/80 px-1 py-0.5 font-mono text-2xs">
                  config/memory-legacy-meta.ts
                </code>
              </p>
              ...
              >
                ...
                >
                  ...
                  <span className="inline text-2xs text-muted-foreground group-data-[state=open]:hidden">
                    Show
                  </span>
                  <span className="hidden text-2xs text-muted-foreground group-data-[state=open]:inline">
                    Hide
                  </span>
                </CollapsibleTrigger>
                <CollapsibleContent
                ...
                >
                  <div className="mt-2 space-y-2 border-t border-border/50 pt-2 text-xs leading-relaxed text-muted-foreground">
                    ...
                    <p>
                      <span className="font-mono text-[11px]">{MEMORY_V2_COLLECTION}</span> supports
                      optional AES-GCM on chunks via{" "}
                      <code className="rounded bg-muted/80 px-1 py-0.5 font-mono text-2xs">
                        MEMORY_ENCRYPTION_*
                      </code>
                      . Fill it with{" "}
              ...
              {!loading && !data && !error ? (
                <ul className="mt-2 text-[12px] text-muted-foreground space-y-0.5 list-disc pl-4 max-w-xl">
                  <li>
                    Queries are sampled from your stored memories when possible, then padded with
                    fixed probe phrases so every run hits the full count.
                  </li>
            ...
            {data ? (
              ...
              >
                ...
                {data.synopsisError ? (
                  <p
                    className="border-t border-border/60 pt-4 text-sm text-destructive"
                    role="status"
                  >
                        ...
                        <section key={`${run.query}-${runIdx}`} className="scroll-mt-4">
                          ...
                          {run.v2Error ? (
                            ...
                            >
                              ...
                              <span>
                                <span className="font-mono text-xs">{MEMORY_V2_COLLECTION}</span>:{" "}
                                {run.v2Error}
                              </span>
                            </div>
                          ) : null}
