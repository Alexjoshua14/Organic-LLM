...
function ControlPanel({
  ...
  return (
    <section
      className={cn(
        "flex min-h-0 flex-col overflow-hidden rounded-xl border border-border/50",
        glass({ opaque: true })
      )}
    >
      ...
      <ul className="divide-y divide-border/30 overflow-y-auto px-2 py-1.5">
        {rows.map((row) => (
          <li
            key={row.label}
            className="flex items-start justify-between gap-4 rounded-lg px-2 py-2.5 transition-colors hover:bg-muted/20"
          >
            <div className="min-w-0 space-y-0.5">
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent
        ...
        className={cn(
          "max-h-[min(88dvh,720px)] gap-0 overflow-hidden border-border/60 p-0 sm:max-w-3xl",
          glass({ opaque: true }),
          "shadow-[0_24px_80px_-24px_rgb(0_0_0/0.45)]"
        )}
      >
        <DialogHeader className="space-y-3 border-b border-border/40 px-6 py-5 text-left">
          <p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground/70">Ergon</p>
          <div className="space-y-1.5">
            <DialogTitle className="font-commissioner text-2xl font-light tracking-wide">
              Controls &amp; shortcuts
            </DialogTitle>
            <DialogDescription className="max-w-prose text-sm leading-relaxed text-muted-foreground">
              Navigate tasks quickly from the keyboard on desktop, or use touch gestures on mobile.
            </DialogDescription>
          </div>
        </DialogHeader>

        <div className="grid min-h-0 gap-4 overflow-y-auto px-4 py-4 sm:grid-cols-2 sm:px-6 sm:pb-6">
          <ControlPanel
            icon={Keyboard}
            rows={KEYBOARD_ROWS}
