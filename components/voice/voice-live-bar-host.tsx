 ...
 * absent — Arcadia, Strata, rabbit holes — it renders into the page-area anchor inside `<main>`
 * (`VoiceLiveBarPageAnchor`), because "the mic is open" must not be a fact that some routes
 * forget to mention. The viewport anchor is only a last resort if neither is mounted.
 *
 ...
 */
export function VoiceLiveBarHost({
  container,
  pageAnchor,
}: {
  ...
  if (pageAnchor) return createPortal(bar, pageAnchor);
