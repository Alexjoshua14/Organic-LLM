...
import { SparkDemoPanel } from "./spark-demo-panel";
...
/**
 * Admin editor for an authored spark: tweak the system prompt + kickoff, then run a demo.
 * The demo runs **in this same modal pane** (swap-views): clicking Demo replaces the editor
 * with a condensed transcript; "← Edit" returns. Edits here are in-memory only (for
 * trialling) — persist by editing `lib/sandbox/noesis/sparks/registry.ts` + `docs/noesis-sparks.md`.
 */
export function SparkEditorDialog({
  ...
  const [view, setView] = useState<"edit" | "demo">("edit");
  ...
  // Always (re)open on the edit view; a fresh open shouldn't land mid-demo.
  useEffect(() => {
    setView("edit");
  }, [open, spark]);
        ...
        {view === "edit" ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Tweak the system prompt and kickoff, then run a demo in place. Edits here are for
                trialling only; persist changes in{" "}
          ...
          <>
            {/* Keep an accessible title for the dialog; the panel shows the visible one. */}
            <DialogTitle className="sr-only">Demo — {spark.userFacingText}</DialogTitle>
            <SparkDemoPanel
              kickoff={kickoff}
              spark={spark}
